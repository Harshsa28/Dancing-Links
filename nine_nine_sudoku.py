import numpy as np
import nine_nine_dlx
import nine_nine_general_matrix
...
def final(take_out_rows, solution):
    ...
    x = [i for i in range(0,729) if i not in take_out_rows]
    for i in solution:
        #row_number = int(x[i] / 16)
        row_number = int(x[i] / 81)
        #column_number = int((x[i] % 16) / 4)
        column_number = int((x[i] % 81) / 9)
        #value = int(x[i] % 4) + 1
        value = int(x[i] % 9) + 1
...
def solve():
    ...
    sudoku = np.array([[4,0,0,0,0,0,0,6,0],
                       [5,0,0,0,3,2,0,0,0],
                       [9,1,0,0,0,4,0,0,0],
                       [0,0,0,6,0,0,4,5,0],
                       [2,0,0,0,1,5,0,0,6],
                       ...
                       [0,0,0,8,0,0,5,4,0],
                       [0,7,0,0,0,0,1,0,0]])
    matrix = nine_nine_general_matrix.give_constrained_matrix()
    cover_rows = []
    for i in range(len(sudoku)):
        for j in range(len(sudoku[i])):
            if sudoku[i][j] != 0:
                ...
                row_number_in_matrix = i * 81 + j * 9 + value - 1
    ...
    solution = nine_nine_dlx.solve(matrix)
